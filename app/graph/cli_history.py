"""从外部 CLI（如 Claude Code）读取节点会话历史并转为 Harness Message。"""
import asyncio
import json
import logging
import os
...
def encode_claude_project_dir(cwd: str) -> str:
    """Claude Code 将工作目录编码为 projects 子目录名。"""
    text = str(Path(cwd).expanduser().resolve())
    encoded = _CLAUDE_PROJECT_ENCODE_RE.sub("-", text).strip("-")
    return encoded or "default"


def resolve_claude_config_dir(history_cfg: GraphNodeCliHistoryConfig) -> Path:
    """解析 Claude Code 配置根目录：节点 config_dir > CLAUDE_CONFIG_DIR > ~/.claude。"""
...
def resolve_claude_jsonl_path(
    config_dir: Path,
    workspace: str,
    session_id: str,
) -> Optional[Path]:
    """定位 Claude Code 会话 JSONL 文件。"""
    ...
    if ws:
        encoded = encode_claude_project_dir(ws)
...
def _messages_from_user_payload(
    ...
    """Claude Code 常把 tool_result 放在 user 消息的 content 数组里，需拆成 tool 消息。"""
    out: List[Message] = []
    ...
    for block in content_raw:
        ...
        if block_type == "tool_result":
            ...
            if isinstance(tool_content, list):
                body = _extract_text_content(tool_content)
            else:
...
def parse_claude_jsonl(path: Path) -> List[Message]:
    """将 Claude Code JSONL 转为 Harness 消息列表（user/assistant/tool）。"""
        ...
        for line_no, line in enumerate(fh, start=1):
            ...
            if record_type == "user":
                ...
                if role != "user":
                    continue
                messages.extend(
                    _messages_from_user_payload(payload, record, tool_names, created_at)
                )
            elif record_type == "assistant":
                payload = record.get("message") if isinstance(record.get("message"), dict) else record
                content_raw = payload.get("content")
                if isinstance(content_raw, list):
                    text, tool_calls = _tool_use_to_tool_calls(content_raw)
                    if tool_calls:
                ...
                tool_content = _extract_text_content(result.get("content"))
                if not tool_content:
                    continue
                messages.append(
...
def resolve_cli_history_config(cli_cfg: GraphNodeCliConfig) -> GraphNodeCliHistoryConfig:
...
class CliHistoryReader:
    """按节点 CLI 配置从第三方系统拉取会话历史。"""

    @staticmethod
    async def fetch_node_messages(
        ...
        if cli_cfg is None:
            raise ValueError("节点缺少 CLI 配置")
        history_cfg = resolve_cli_history_config(cli_cfg)
        if history_cfg.provider == "none":
            raise ValueError("该 CLI 节点未启用历史读取")
        sid = (cli_session_id or "").strip()
        if not sid:
        ...
        cwd = CliNodeExecutor._resolve_cli_cwd(cli_cfg.cwd, workspace_path)
        config_dir = resolve_claude_config_dir(history_cfg)
        jsonl_path = resolve_claude_jsonl_path(config_dir, cwd, sid)
        if jsonl_path is None:
            raise FileNotFoundError(
                f"未找到 Claude Code 会话文件（session={sid}，workspace={cwd}）"
            )
        return await asyncio.to_thread(parse_claude_jsonl, jsonl_path)
